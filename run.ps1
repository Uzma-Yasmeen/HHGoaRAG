...
#   .\run.ps1  # final 50-answerable + 50-unanswerable evaluation
#   .\run.ps1 --num-answerable 1 --num-unanswerable 1  # quick smoke test
...
# GoaVaani exposes the evaluator's native interface through this adapter.
# Callers can still override either variable explicitly when needed.
if (-not $env:EVAL_EMBEDDER_MODULE) {
    $env:EVAL_EMBEDDER_MODULE = "backend.app.eval_target"
}
if (-not $env:EVAL_GENERATOR_MODULE) {
    $env:EVAL_GENERATOR_MODULE = "backend.app.eval_target"
}

# Keep the common path one-command and resilient to retired free-model IDs.
# An explicitly configured custom model is preserved; only missing or known
# stale defaults are replaced by OpenRouter's capability-aware free router.
if (-not $env:EVAL_JUDGE_PROVIDER -or $env:EVAL_JUDGE_PROVIDER -eq "openrouter") {
    $env:EVAL_JUDGE_PROVIDER = "groq"
}
if (
    -not $env:EVAL_JUDGE_MODEL_OPENROUTER -or
    $env:EVAL_JUDGE_MODEL_OPENROUTER -in @("stealth/ox-alpha", "openai/gpt-oss-20b:free")
) {
    $env:EVAL_JUDGE_MODEL_OPENROUTER = "openrouter/free"
}
