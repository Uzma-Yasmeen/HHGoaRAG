...
def run(results: list[ExampleResult], workers: int) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        ...
        for future in as_completed(futures):
            ...
            except Exception as exc:  # one hosted-judge failure must not lose the local eval
                errors += 1
    ...
    if errors and not verdicts:
        return {
            ...
            "num_evaluated": 0,
            "error": f"Hosted judge unavailable ({', '.join(sorted(error_types))}); check skipped.",
        }
    if not verdicts:
    ...
    if errors:
        report["warning"] = f"{errors} hosted judge calls failed ({', '.join(sorted(error_types))})."
    return report
