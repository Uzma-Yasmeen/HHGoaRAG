...
from eval import dataset, index_build, judge, pipeline, target
...
def main():
    ...
    parser.add_argument("--num-answerable", type=int, default=50, help="answerable MSMARCO-XI rows to sample (default: 50)")
    parser.add_argument("--num-unanswerable", type=int, default=50, help="unanswerable rows to sample (default: 50)")
    parser.add_argument("--top-k", type=int, default=5, help="results retrieved per query (default: 5)")
    parser.add_argument("--workers", type=int, default=1, help="parallel workers for retrieval+generation (default: 1; safe for local/shared models)")
    parser.add_argument("--judge-workers", type=int, default=2, help="parallel workers for judge calls, per check (default: 2; conservative for hosted/free APIs)")
    ...
    print("\nChecking hosted judge availability...")
    judge_error = None
    probe = next((r for r in results if r.error is None and r.answer_text), None)
    if probe is not None:
        try:
            judge.judge_faithfulness(probe.answer_text, probe.context_text_en)
            print("Hosted judge is ready.")
        except Exception as exc:  # local checks should still be reported and saved
            judge_error = f"Hosted judge unavailable ({type(exc).__name__}: {exc})"
            print(f"[warning] {judge_error}. Judge-based checks will be skipped.")
    ...
    try:
        ...
        f_faithfulness = None if judge_error else pool.submit(faithfulness.run, results, args.judge_workers)
        f_correctness = None if judge_error else pool.submit(correctness.run, results, args.judge_workers)

        retrieval_report = f_retrieval.result()
        reliability_report = f_reliability.result()
        latency_report = f_latency.result()
        faithfulness_report = (
            f_faithfulness.result()
            if f_faithfulness
            else {
        ...
        print("\nEvaluation cancelled.", file=sys.stderr, flush=True)
        os._exit(130)
    else:
